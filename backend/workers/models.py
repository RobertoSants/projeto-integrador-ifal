...
class Worker(models.Model):
    ...
    photo = models.TextField(blank=True, null=True) # Alterado para TextField para receber string Base64
    services = models.ManyToManyField(ServiceCategory, blank=True, related_name="workers")
    created_at = models.DateTimeField(auto_now_add=True)

    @property
    def age(self):
        """Calcula dinamicamente a idade atual baseada no ano corrente"""
        today = date.today()
